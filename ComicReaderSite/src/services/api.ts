...
const DEFAULT_API_BASE_CANDIDATES = [
  ...
  // Common local Tomcat context used by this project
...
if (typeof window !== "undefined") {
  ...
  // Only add an origin candidate when we're running under HTTP(S).
  // Avoid adding chrome-extension:// or other non-http schemes which produce
  // invalid fetch URLs like "chrome-extension://...:8080" and cause
  // "Failed to fetch" TypeError in the browser.
  if (protocol === "http:" || protocol === "https:") {
    const origins = [`${protocol}//${hostname}:8080/api`];
    candidateBaseUrls.push(...origins);
  } else {
    console.debug(
      `[ApiService] Skipping adding origin candidate for protocol=${protocol}`
    );
  }
}
...
class ApiService {
  ...
  // Debug helper: return candidate base URLs and the currently resolved URL
  // without performing any network requests. Useful to call from DevTools.
  debugResolve(): { candidates: string[]; resolved: string | null } {
    return {
      candidates: [...this.candidateBaseUrls],
      resolved: this.resolvedBaseUrl,
    };
  }
  ...
  private async resolveBaseUrl(): Promise<string> {
    ...
    this.resolvingPromise = (async () => {
      console.debug(
        "[ApiService] Resolving backend base URL from candidates:",
        this.candidateBaseUrls
      );
      for (const baseUrlCandidate of this.candidateBaseUrls) {
        // Skip any candidate that doesn't look like an http(s) URL. Some
        // environments (browser extensions, file://) expose non-http schemes
        // which will cause fetch() to throw a TypeError immediately.
        if (!/^https?:\/\//i.test(baseUrlCandidate)) {
          console.debug(
            `[ApiService] Skipping non-http candidate: ${baseUrlCandidate}`
          );
          continue;
        }
        try {
          console.debug(`[ApiService] Probing ${baseUrlCandidate}/health`);
          const response = await fetch(`${baseUrlCandidate}/health`, {
            method: "GET",
            mode: "cors",
          });
          if (response.ok) {
            ...
            console.info(
              `[ApiService] Using backend base URL: ${baseUrlCandidate}`
            );
            return baseUrlCandidate;
          }
        } catch (error) {
          console.warn(
            `API base URL candidate failed (${baseUrlCandidate}/health):`,
            error
          );
        }
      }
      ...
      console.warn(
        `[ApiService] Falling back to first candidate base URL: ${fallback}`
      );
      return fallback;
    })().finally(() => {
  ...
  private async request<T>(
    ...
    try {
      ...
      if (error instanceof TypeError) {
        const hint =
          "The browser blocked this request. Double-check that the backend is running and that the URL includes the correct Tomcat context path (for example http://localhost:8080/ComicDashboard-1.0-SNAPSHOT/api).";
  ...
  async login(username: string, password: string, bookmarks?: any[]) {
...
// Expose the service on window in development for quick debugging from DevTools
if (
