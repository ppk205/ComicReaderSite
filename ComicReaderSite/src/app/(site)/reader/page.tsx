...
const isCoverItem = (x: SpineItem) => {
  ...
  const idref = (x.idref || '').toLowerCase();
  const props = (x.props || '').toLowerCase();
  return (
    href.includes('cover') ||
...
export default function ReaderPage() {
  ...
  const handleNext = useCallback(async () => {
    ...
    // Nếu đang ở cover → nhảy sang nội dung
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const res = await fetch(`${API_BASE}/epub/file?id=${encodeURIComponent(bookId)}`);
        ...
        const items: SpineItem[] = (book.spine?.items || []).map((it: Section, i: number) => ({
          index: i,
          href: it.href || '',
          idref: it.idref || '',
          props: Array.isArray(it.properties) ? it.properties.join(' ') : it.properties || '',
        }));
        ...
        rendition.on('displayed', (view: View) => {
          const doc =
            (view as any)?.document ||
            (view as any)?.contents?.document ||
            null;
