...
export default function CommunityPage() {
    ...
    return (
        <main className="max-w-6xl mx-auto p-8 bg-sky-50 text-sky-900 rounded-2xl shadow-lg mt-8 border border-sky-100 font-sans">
            <div className="flex justify-between items-center mb-8 border-b border-sky-200 pb-3">
                <h1 className="text-3xl font-extrabold text-sky-700">🌤 Community</h1>
                <Link
            ...
            {err && <p className="text-red-600 bg-red-50 border border-red-200 p-3 rounded-lg mb-4">⚠️ {err}</p>}
                    ...
                    {posts.map((p) => (
                        ...
                        >
                            ...
                            <div className="flex-1 p-4 flex flex-col justify-between">
                                <div>
                                    <h2 className="text-lg font-semibold text-sky-800 line-clamp-2">{p.title}</h2>
                                    <p className="text-sm text-sky-700 mt-2 line-clamp-3">
                                        {p.content ? p.content.slice(0, 100) + '…' : 'Không có nội dung.'}
                                    </p>
                                </div>
                                <p className="text-xs text-sky-500 mt-3">
                                    👤 {p.authorId ?? '—'} •{' '}
                                    {p.createdAt ? new Date(p.createdAt).toLocaleDateString('vi-VN') : 'Không rõ thời gian'}
                                </p>
                            </div>
                        </Link>
                    ))}
